...
StringPoolPointer insert_string(StringPool *string_pool, const char *string, const size_t string_len) {
    StringPoolPointer return_pointer = {.status = NO_ERROR, .string_length = 0, .string_start = NULL};
    ...
    if (string_pool->used + string_len >= string_pool->capacity) {
        Status reallocation_status = reallocate_string_pool(string_pool, string_pool->capacity*2);
    ...
    memcpy(string_pool->start_pointer + string_pool->used, string, string_len);
